    ...
    public class ConverterViewModel : INotifyPropertyChanged
{
    ...
    private void ConvertCurrency(bool fromSource)
    {
        ...
        try
        {
            ...
            if (fromSource)
            {
                if (decimal.TryParse(AmountFrom, out decimal amount))
                {
                    AmountTo = ((amount * fromRate) / toRate * SelectedFromCurrency.Nominal).ToString("F2");
                }
            }
            else
            {
                ...
                {
                    AmountFrom = ((amount * toRate) / fromRate * SelectedToCurrency.Nominal).ToString("F2");
                }
            }
        }
        finally
